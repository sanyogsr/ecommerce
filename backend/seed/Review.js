...
const uniqueReviewsMap = new Map(reviews.map((review) => [review._id, review]));
const uniqueReview = Array.from(uniqueReviewsMap.values());
const seedReview = async () => {
  try {
    await Review.deleteMany({});
    await Review.insertMany(uniqueReview);
