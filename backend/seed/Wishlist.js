...
const uniqueWishlistMap = new Map(
  wishlistItem.map((wishlist) => [wishlist._id, wishlist])
);
const uniqueWishlist = Array.from(uniqueWishlistMap.values());

const seedWishlist = async () => {
  try {
    await Wishlist.deleteMany({});

    await Wishlist.insertMany(uniqueWishlist);
